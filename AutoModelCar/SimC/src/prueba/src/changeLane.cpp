...
ros::Publisher evadeA_publisher; 
...
void movimiento(int cont){
	...
	velocity_publisher = nh.advertise<std_msgs::Int16>("/a0/manual_control/speed", 1);//AutoNOMOS_mini/manual_control/speed
        ...
        evadeA_publisher = nh.advertise<std_msgs::Int16>("/evadeA",1);
	std_msgs::Int16 velocity_message;
  	std_msgs::Int16 steering_message;
  	std_msgs::Int16 evading_message;
	//if(cont<220){
 	...
 	evading_message.data=esquiva;
    	if(esquiva==1 or termino ==1){
    		evading_message.data=1;
    	}
    	
    	evadeA_publisher.publish(evading_message);
	/*}
	else{
		if(carril==1){
			velocity_message.data=-80;
