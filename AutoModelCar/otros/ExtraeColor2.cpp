...
void cameraCallback(const std_msgs::Float64MultiArray& msg)
{
  ...
  uint16_t g; 
  uint16_t b;
  red_pub = nh.advertise<std_msgs::UInt16MultiArray>("/red", 1); 
  green_pub = nh.advertise<std_msgs::UInt16MultiArray>("/green", 1); 
  blue_pub = nh.advertise<std_msgs::UInt16MultiArray>("/blue", 1);
  ...
  for (int i = 1; i <= size; i++){
    ...
    std_msgs::UInt16MultiArray red;
    std_msgs::UInt16MultiArray green;
    std_msgs::UInt16MultiArray blue;
    red.data=rV;
    green.data=gV;
    blue.data=bV;
